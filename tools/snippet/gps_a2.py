...
while True:
    ...
    # ---- PRINT RAW SERIAL PAYLOAD ----
    print("=== RAW SERIAL START ===")
    sys.stdout.buffer.write(ubx_pvt)
    sys.stdout.buffer.write(ubx_posllh)
    sys.stdout.buffer.write(ubx_velned)
    sys.stdout.buffer.write(ubx_sol)
    sys.stdout.buffer.write((gga + "\r\n").encode())
    sys.stdout.buffer.write((rmc + "\r\n").encode())
    sys.stdout.flush()
    print("\n=== RAW SERIAL END ===")
    ...
    print("Sent: UBX-NAV-PVT, POSLLH, VELNED, SOL + NMEA GGA, RMC")
    print("-" * 50)
