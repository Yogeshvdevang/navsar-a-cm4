"""MAVLink Sniffer module. Provides mavlink sniffer utilities for NAVISAR."""
...
def main():
    """Stream GPS/LiDAR MAVLink messages to stdout."""
    ...
    while True:
        ...
        if PRINT_INTERVAL_S <= 0.0 or (now - last_print) >= PRINT_INTERVAL_S:
            if msg_type == "GPS_RAW_INT":
                ...
                print(f"GPS: lat={lat:.7f} lon={lon:.7f} alt_m={alt_m:.2f}")
                ...
                print(
                    f"GPS(global): lat={lat:.7f} lon={lon:.7f} alt_m={alt_m:.2f} rel_alt_m={rel_alt_m:.2f}"
                )
            elif msg_type == "DISTANCE_SENSOR":
                dist_m = msg.current_distance / 100.0
                print(f"LiDAR: distance_m={dist_m:.2f}")
            last_print = now
