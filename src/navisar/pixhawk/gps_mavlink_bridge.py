"""Utilities to send GPS input over MAVLink."""
...
def parse_args():
    """Parse CLI arguments for the MAVLink GPS bridge."""
    ...
    parser.add_argument("--lat", type=float, default=80.9716, help="Start latitude (deg)")
    parser.add_argument("--lon", type=float, default=10.5946, help="Start longitude (deg)")
...
def load_gps_origin(config_path=None):
    """Load the GPS origin (lat, lon, alt) from pixhawk.yaml."""
    if config_path is None:
        root = Path(__file__).resolve().parents[2]
        config_path = root / "config" / "pixhawk.yaml"
    else:
    ...
    lat = gps_origin.get("lat")
    lon = gps_origin.get("lon")
    alt = gps_origin.get("alt")
...
def send_fake_gps(args):
    """Stream a synthetic GPS_INPUT message to Pixhawk."""
